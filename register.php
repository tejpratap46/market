...
if (! empty ( $username ) || ! empty ( $password )) {
	...
	$bank = mysql_query ( "INSERT INTO bank (name,bankid,email,balance,lastupdate) VALUES ('$name','$bankid','$email','0','" . date ( "d-m-y H:i:s" ) . "')" ) or die ( "{\"status\":0," . "\"error\":\"" . "Invalid Bankid" /*mysql_error()*/ . "\"}" );
	if ($data && $bank) {
		setcookie ( 'username', $username, time () + (86400 * 30 * 12), "/" );
