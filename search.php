...
<html>
<head>
<title><?php echo $_GET['q'];?> :: One Store</title>
	...
	<div class="container">
		...
		<div class="top-header-nav">
			...
			<div class="top-header-search-box">
				<form>
					<input type="text" name="q" placeholder="Search" required maxlength="22"> <input
						type="submit" value=" ">
				</form>
			</div>
			<!---top-header-search-box--->
			...
			<div class="special-products all-poroducts">
				<div class="s-products-head">
					<div class="s-products-head-left">
						<h3>
							Search <span><?php echo strtoupper($_GET['q']);?></span>
						</h3>
					</div>
				...
				<div class="special-products-grids">
					<?php
					$quary = mysql_query ( "SELECT * FROM `market` WHERE `itemname` LIKE '%" . $_GET ['q'] . "%' OR `itembrand` LIKE '%" . $_GET ['q'] . "%' OR `itemcategory` LIKE '%" . $_GET ['q'] . "%'" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
					if (mysql_num_rows ( $quary ) > 0) {
						for($i = 0; $i < mysql_num_rows ( $quary ); $i ++) {
							$info = mysql_fetch_array ( $quary );
