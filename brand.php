...
<html>
<head>
<title><?php echo $_GET['brand']." :: ".$_GET['c'];?> :: One Store</title>
				...
				<div class="product-filter">
					...
					<div class="product-filter-grids">
						<div class="col-md-3 product-filter-grid1-brands">
							...
							<?php
							if ($_GET ['c']) {
								$brandsQuery = mysql_query ( "SELECT DISTINCT itembrand FROM market WHERE itemcategory='" . $_GET ['c'] . "' ORDER BY `totalsold` DESC" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
								$brandSearch = "brand.php?c=" . $_GET ['c'] . "&";
							} else {
								$brandsQuery = mysql_query ( "SELECT DISTINCT itembrand FROM market ORDER BY `totalsold` DESC" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
								$brandSearch = "brand.php?";
							}
							for($i = 0; $i < mysql_num_rows ( $brandsQuery ); $i ++) {
								...
								if ($i % 2 == 0) {
									...
									echo "<li><a href='" . $brandSearch . "brand=" . $brandInfo ['itembrand'] . "'>" . $brandInfo ['itembrand'] . "</a></li>";
			...
			<div class="special-products all-poroducts">
				<div class="s-products-head">
					<div class="s-products-head-left">
						<h3>
							Brand <span><?php echo strtoupper($_GET['brand']);?></span>
						</h3>
					</div>
					<div class="s-products-head-right" hidden>
						<a href="topselling.php"><span> </span>view all products</a>
					</div>
					<div class="clearfix"></div>
				</div>
				...
				<div class="special-products-grids">
					<?php
					if ($_GET ['c'] && $_GET ['brand']) {
						$query = mysql_query ( "SELECT * FROM market WHERE itemcategory='" . $_GET ['c'] . "' AND itembrand='" . $_GET ['brand'] . "' ORDER BY `totalsold` DESC LIMIT 12" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
						showData ( $query );
					} else if ($_GET ['brand']) {
						$query = mysql_query ( "SELECT * FROM market WHERE itembrand='" . $_GET ['brand'] . "' ORDER BY `totalsold` DESC LIMIT 12" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
						showData ( $query );
					} else {
						echo '<p>Select One Brand</p>';
					}
					?>
					<div class="clearfix"></div>
				</div>
