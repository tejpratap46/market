			...
			<div class="special-products all-poroducts">
				<div class="s-products-head">
					<div class="s-products-head-left">
						<h3>
							COUPON <span><?php echo strtoupper($_GET['coupon']);?></span>
						</h3>
					</div>
				...
				<div class="special-products-grids">
					<?php
					$couponQuary = mysql_query ( "SELECT `items` FROM `coupon` WHERE `code` = '" . $_GET ['coupon'] . "'" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
					$couponArray = mysql_fetch_array ( $couponQuary );
					$couponItems = $couponArray ['items'];
					if (strlen ( $couponItems ) > 0) {
						$couponItems = str_getcsv ( $couponItems );
						for($i = 0; $i < count ( $couponItems ); $i ++) {
							$query = mysql_query ( "SELECT * FROM market WHERE itemid='" . $couponItems [$i] . "'" ) or die ( "{\"status\":0," . "\"error\":\"" . mysql_error () . "\"}" );
							$info = mysql_fetch_array ( $query );
